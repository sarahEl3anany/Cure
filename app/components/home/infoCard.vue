  ...
  <div class="w-full space-y-3">
    ...
    >
      ...
      <div class="flex flex-col flex-1 text-left space-y-2 sm:space-y-0 sm:pr-2">
        ...
        <p class="font-montserrat text-sm sm:text-xs text-neutral-700 truncate">
          {{ value.specialty }} | {{ value.location }}
        </p>

        <div class="flex items-center gap-3 mt-1">
          <div class="flex items-center gap-1">
            <i class="pi pi-star-fill text-yellow-400 text-sm sm:text-xs"></i>
            <span class="font-montserratMedium text-sm sm:text-xs text-secondary-400">
              {{ value.rating }}
            </span>
          </div>

          <div class="flex items-center gap-1">
            <i class="pi pi-clock text-neutral-400 text-sm sm:text-xs"></i>
            <span class="font-montserratMedium text-sm sm:text-xs text-secondary-400">
              {{ value.time }}
            </span>
          </div>
